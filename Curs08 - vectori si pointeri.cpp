...
void main() {
	...
	Scara* scariInline = new Scara[3]{ s1,s2,s3 };
	delete []scariInline;


	Scara** vp = new Scara * [3];
	vp[0] = &s1;
	vp[1] = &s2;
	vp[2] = &s3;
	...
	Scara* pointerScara = new Scara();
	Scara* vector = new Scara[5];

	Scara** pointeri = new Scara * [5];
	for (int i = 0; i < 5; i++) {
		pointeri[i] = new Scara(50+i, 30+i);
	}

	Scara** matrice = new Scara * [5];
	for (int i = 0; i < 5; i++) {
		matrice[i] = new Scara[5];
	}
